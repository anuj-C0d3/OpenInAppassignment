...
class TopLinkRvAdapter(private val context: Context, private val topLinkList: List<TopLink>):RecyclerView.Adapter<TopLinkRvAdapter.ViewHolder>(
   ...
   inner class ViewHolder(val binding: ToplinkitviewBinding):RecyclerView.ViewHolder(binding.root)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TopLinkRvAdapter.ViewHolder {
        val binding = ToplinkitviewBinding.inflate(LayoutInflater.from(context),parent,false)
        return ViewHolder(binding)
    }


    override fun onBindViewHolder(holder: TopLinkRvAdapter.ViewHolder, position: Int) {
